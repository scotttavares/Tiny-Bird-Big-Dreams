...
| `Validation failed (409) … built with the iOS NN SDK. All … apps must be built with the iOS MM SDK or later` at the Publishing/upload step | Codemagic's Xcode (its bundled iOS SDK) is older than Apple's currently-required minimum SDK | Bump `environment.xcode` in `codemagic.yaml` — use `latest` so it tracks Codemagic's newest stable Xcode. If the newer Xcode then fails to *compile* (RN/Expo too old for that toolchain), upgrade the Expo SDK to the version that targets that iOS SDK |
| Main steps incl. **Publishing** ✅ but a **post-processing "App Store distribution"** step ❌ (build shows red) | `submit_to_testflight: true` runs an auto-distribute step that waits on Apple's processing / a beta group and fails the build — even though the upload already succeeded and the build is in TestFlight | The upload is done regardless; check App Store Connect → TestFlight ("Validated / Ready to Submit"). For green builds, omit `submit_to_testflight` (upload-only) and use Internal Testing (auto-distributes, no review). Re-add it with a `beta_groups:` only for external auto-distribution |
| Signing fails on the **widget** extension, or `... doesn't include the com.apple.security.application-groups entitlement` | The widget is a second bundle id (`<app>.widget`) needing its own profile, and the App Group must be enabled on both App IDs | `codemagic.yaml` already runs `fetch-signing-files --create` for both `$BUNDLE_ID` and `$BUNDLE_ID.widget`. In the Apple portal, create the App Group once and enable "App Groups" on both App IDs (see orbit/README.md), then re-run |
| Archive signing fails on `aps-environment` after adding **expo-notifications** | Expo's prebuild auto-applies expo-notifications' iOS plugin (just from being installed), adding a push entitlement — but local notifications don't need it | Keep the `plugins/withLocalNotificationsOnly.js` config plugin (last in `app.json` plugins) that strips `aps-environment`; verify with `npx expo prebuild` → `grep aps-environment ios/Orbit/Orbit.entitlements` (should be empty) |
